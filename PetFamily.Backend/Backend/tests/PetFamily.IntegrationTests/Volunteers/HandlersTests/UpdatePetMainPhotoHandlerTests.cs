...
public class UpdatePetMainPhotoHandlerTests : VolunteerTestsBase
{
    ...
    public UpdatePetMainPhotoHandlerTests(VolunteerTestsWebFactory factory)
        : base(factory)
    {
    ...
    public async Task UploadPetMainPhoto_success_should_make_selected_photo_main_if_there_were_no_main_photo()
    {
        ...
        var newMainPhoto = photos[1].Id;

        var command = new UpdatePetMainPhotoCommand(volunteer.Id, pet.Id, newMainPhoto);
        ...
        var main_photos = pet!.PhotosList.Where(photo => photo.Main).ToList();
        main_photos.Count.Should().Be(1);
        main_photos[0].Id.Should().Be(newMainPhoto);
    }
    ...
    public async Task UploadPetMainPhoto_failure_requested_main_photo_doesnt_exist_in_database()
    {
        ...
        pet.UpdateMainPhoto(photos[0]);
        await VolunteersWriteDbContext.SaveChangesAsync();
        var newMainPhoto = Guid.NewGuid();
        var oldMainPhoto = photos[0].Id;
        ...
        var main_photos = pet!.PhotosList.Where(photo => photo.Main).ToList();
        main_photos.Count.Should().Be(1);
        main_photos[0].Id.Should().Be(oldMainPhoto);
    }
}