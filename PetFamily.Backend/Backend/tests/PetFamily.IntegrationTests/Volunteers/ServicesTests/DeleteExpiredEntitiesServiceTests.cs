...
public class DeleteExpiredEntitiesServiceTests : VolunteerTestsBase
{
    ...
    public DeleteExpiredEntitiesServiceTests(VolunteerTestsWebFactory factory)
        : base(factory)
    {
    ...
    public async Task SoftDeletePet_success_should_delete_2_pets_that_were_soft_deleted_with_expired_lifetime()
    {
        ...
        const int DAYS_BEFORE_DELETION = 0; // should be deleted immediately
    ...
    public async Task SoftDeletePet_success_should_delete_only_1_pet_from_2_soft_deleted_because_of_their_lifetime()
    {
        // arrange
        const int PET_COUNT = 5;
        const int DAYS_BEFORE_DELETION = 3;
        const string PRIVATE_PROPERTY_NAME = "DeletionDate";
        var volunteer = await DataGenerator.SeedVolunteerWithPets(
            VolunteersWriteDbContext, SpeciesWriteDbContext, PET_COUNT);
        var pet1 = volunteer.AllOwnedPets[0];
        var pet2 = volunteer.AllOwnedPets[3];
    ...
    public async Task SoftDeletePet_success_should_delete_volunteer_with_expired_lifetime_and_all_its_pets()
    {
        ...
        const int DAYS_BEFORE_DELETION = 3;
        const string PRIVATE_PROPERTY_NAME = "DeletionDate";
        var volunteer1 = await DataGenerator.SeedVolunteerWithPets(
            VolunteersWriteDbContext, SpeciesWriteDbContext, PET_COUNT);
        var volunteer2 = await DataGenerator.SeedVolunteerWithPets(
            VolunteersWriteDbContext, SpeciesWriteDbContext, PET_COUNT);
        volunteer1.Delete();
        volunteer2.Delete();
    ...
    private static void SetPrivatePropertyBase<T>(T obj, string propertyName, object value)
    {
